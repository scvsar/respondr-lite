...
"""
	...
	def _create_with_fallback():
		...
		# last resort
		return client.chat.completions.create(model=model, messages=messages_payload, max_completion_tokens=220), messages_payload, {"max_completion_tokens": 220}, "minimal"
	...
	try:
		...
		log_entry.update({
			"response": {"status": "success", "raw": raw_content[:300], "response_format": chosen_format, "kwargs_used": used_kwargs},
			"parsed_data": data,
		})
		try:
			with open(log_file, "a", encoding="utf-8") as f:
...
def get_current_status(request: Request) -> JSONResponse:
	...
	sorted_messages = sorted(messages, key=lambda x: x.get('timestamp_utc', x.get('timestamp', '')))
