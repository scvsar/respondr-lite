...
# Apply replacements for computed placeholders (always replace; treat null as empty)
$rep = if ($null -eq $oidcTenantSegment) { '' } else { [string]$oidcTenantSegment }
$processedContent = $processedContent -replace [regex]::Escape('{{OIDC_TENANT_SEGMENT}}'), $rep

$rep = if ($null -eq $emailArgs) { '' } else { [string]$emailArgs }
$processedContent = $processedContent -replace [regex]::Escape('{{EMAIL_DOMAIN_ARGS}}'), $rep

$rep = if ($null -eq $ALLOWED_EMAIL_DOMAINS_PLACEHOLDER) { '' } else { [string]$ALLOWED_EMAIL_DOMAINS_PLACEHOLDER }
$processedContent = $processedContent -replace [regex]::Escape('{{ALLOWED_EMAIL_DOMAINS_PLACEHOLDER}}'), $rep

$rep = if ($null -eq $ALLOWED_ADMIN_USERS_PLACEHOLDER) { '' } else { [string]$ALLOWED_ADMIN_USERS_PLACEHOLDER }
$processedContent = $processedContent -replace [regex]::Escape('{{ALLOWED_ADMIN_USERS_PLACEHOLDER}}'), $rep
